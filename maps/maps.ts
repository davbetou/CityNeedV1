...
import {FirebaseDetailsAssociationPage} from "../firebase-integration/firebase-details-association/firebase-details-association";
import {FirebaseAllAssociationsPage} from "../firebase-integration/firebase-all-associations/firebase-all-associations";
import {FirebaseAllCompaniesPage} from "../firebase-integration/firebase-all-companies/firebase-all-companies";
import {FirebaseAllPromotionsPage} from "../firebase-integration/firebase-all-promotions/firebase-all-promotions";
import {FirebaseAllSuggestions} from "../firebase-integration/firebase-all-suggestions/firebase-all-suggestions";
import {FirebaseAllDonsPage} from "../firebase-integration/firebase-all-dons/firebase-all-dons";
...
export class MapsPage implements OnInit {
  ...
  toast: any;
    items: Array<any>;
    duration_filtered_items: Array<any>;
  ...
  constructor(
    ...
    let that = this;
      ...
      fAuthService.getProfil().then(profil =>{
         console.log( 'profil dans map',profil );
         that.userprofil = profil;

         fAuthService.checkisclient(profil);
     })
...
console.log('hello');
    ...
    env.GoogleMapsService.getPlacesNearby(location).subscribe(
      nearby_places => {
        ...
        env.map_model.map.fitBounds(bound);
          console.log( this.map_model.nearby_places );
  ...
  goToAides(){
      this.nav.push(FirebaseAllDonsPage);
}

  goToPromo(){
      this.nav.push(FirebaseAllPromotionsPage);
}

  goToSuggestions(){
      this.nav.push(FirebaseAllSuggestions);
}
